...
public class AssignmentPart1
{
	...
	public static void main(String[] args)
    {
        ...
        while (time < timerLength)
        {
            ...
            for (int s = 0; s < numSacks; s++)
            {
                giftsInSacks += sacks[s].current_index;
            }
            
			// Get total count from all the sacks
			

            int giftsInHoppers = 0;
            // TODO - calculate number
			
			// Get total count from all the hoppers
            for (int h = 0; h < numHoppers; h++)
            {
            	giftsInHoppers += hoppers[h].current_index - hoppers[h].start_index;
            }
        ...
        for (int h = 0; h < numHoppers; h++)
        {
        	giftsDeposited += hoppers[h].start_index;
        }
        ...
        for (int h = 0; h < numHoppers; h++)
        {
            System.out.println("Hopper " + hoppers[h].id + " deposited " + hoppers[h].start_index + 
            				   " presents and waited " + hoppers[h].wait_time/1000 + "s.");
        }
        System.out.println();

        int giftsOnMachine = 0;
        
        // Turn table & Conveyer Belt
        
        // Turn table
        for (int t = 0; t < numTurntables; t++)
        {
            if(tables[t].getCurr_present() != null) giftsOnMachine++;
        }
